...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        for i in range(len(sentence)):
            #get index in sentence for the left side
            left_index = [-1 if x <0 else x for x in range(i-1,i-window_size-1,-1)]
            left_index.reverse()
            #get index in sentence for the right side
            right_index = [-2 if x >len(sentence)-1 else x for x in range(i+1,i+1+window_size,1)]

            indexs = left_index+[i]+right_index

            #interprete the indexs
            vector = []
            for index in indexs:
                if index == -1:
                    vector.extend(start)
                    ...
                    vector.extend(sentence[index])

            #append the data into the window data
            windowed_data.append((vector,labels[i]))
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        #THIS ARE CHANGED REFERING TO https://github.com/hankcs/CS224n/blob/master/assignment3/q1_window.py
        #THIS SHOULD BE NOTED LATER IN SUMMARY
        embedded = tf.Variable(self.pretrained_embeddings)

        lookup_result = tf.nn.embedding_lookup(embedded,self.input_placeholder)
        embeddings = tf.reshape(tensor = lookup_result, shape = [-1,n_features*embedding_size])                                                                                                                                      
        ### END YOUR CODE
        return embeddings
    ...
    def add_loss_op(self, pred):
        ...
        #map the result holder
        label_one_hot_matrix = tf.one_hot(self.labels_placeholder,depth=self.config.n_classes)
        entropy_matrix = tf.nn.softmax_cross_entropy_with_logits(labels = label_one_hot_matrix,logits = pred)
        loss = tf.reduce_mean(entropy_matrix)
        ### END YOUR CODE
        return loss
