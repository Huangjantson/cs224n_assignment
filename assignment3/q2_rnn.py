...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        if valid_length < max_length:
            invalid_length = max_length - valid_length
            filled_sentence = sentence + ([zero_vector] * invalid_length)
            filled_labels = labels + ([zero_label] * invalid_length)
            ret.append([filled_sentence, filled_labels, [True] * valid_length + [False] * invalid_length])
        else:
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        lookup_result = tf.nn.embedding_lookup(embedding_matrix,self.input_placeholder)
        embeddings = tf.reshape(tensor = lookup_result, shape = [-1,Config.max_length,Config.n_features*Config.embed_size])       
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        with  tf.variable_scope('Layer2'):
            ...
            b2 =  tf.get_variable("b2",shape = [Config.n_classes,],initializer = 
                tf.contrib.layers.xavier_initializer())
        input_shape = tf.shape(x)
        state = tf.zeros([input_shape[0],Config.hidden_size],dtype = tf.float32)
        ### END YOUR CODE
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                hidden, state = cell(x[:,time_step,:], state,scope = "RNN")
                h_drop = tf.nn.dropout(hidden,keep_prob = Config.dropout)
                temp_pred = tf.matmul(h_drop,U) + b2
...
def do_test2(args):
    ...
    for i, (sentence, labels) in enumerate(dev):
            assert(len(sentence)==len(labels))
